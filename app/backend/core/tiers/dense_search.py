...
fusion RRF (Reciprocal Rank Fusion). BO KEYFRAME BTC GOC (filter/tier2_vector/resources.py
...
duoi) dung nhu HARD FILTER truoc khi rank (giong filter.by_text() nhung scope o bo dense,
...
def _load_dense_meta() -> pd.DataFrame:
    """2026-08-21 (theo yeu cau nguoi dung: path "không được hard, mà có thể linh động giữa
    nhiều máy, nhiều cấu trúc dự án khác nhau") - dense_meta.parquet["path"] tren dia GIO chi
    luu duong dan TUONG DOI ("<prefix>/<video_id>/<filename>.jpg", xem config.KEYFRAME_ROOT) -
    noi voi goc that cua may hien tai NGAY LUC DOC (1 lan duy nhat, cache lai qua lru_cache) de
    moi noi goi ben duoi (vd `meta["path"].values` o _rank_single_remote) van nhan duoc duong
    dan TUYET DOI nhu truoc, khong phai sua gi them."""
    meta = pd.read_parquet(DENSE_META_PATH)
    meta["path"] = [str(KEYFRAME_ROOT / rel) for rel in meta["path"]]
    return meta
...
def _object_candidates(
    ...
    """Giong filter.by_objects() nhung key la (video_id, frame_idx) va scope LUON la
...
def _ocr_frame_words() -> dict[tuple[str, int], list[str]]:
    ...
    _flexible_word_match() (xem filter.py) de vua khoan dung box khac chen giua, vua
...
def _ocr_match_candidate_keys(needle_words: list[str], algorithm: str) -> set[tuple[str, int]]:
    ...
    filter.OCR_MATCH_ALGORITHMS). CHI dung prefilter substring (_ocr_prefilter_candidates)
...
def _ocr_candidates(ocr_text: str, algorithm: str = DEFAULT_OCR_ALGORITHM) -> set[tuple[str, int]] | None:
    ...
    algorithm: key trong filter.OCR_MATCH_ALGORITHMS (xem _ocr_match_candidate_keys ve
...
# OCR") - GIONG HET pattern OCR_MATCH_ALGORITHMS (filter.py): registry + dispatch, "cosine"
...
def _encode_query(query: str, model: str) -> np.ndarray:
    ...
    # con cach nao toi uu them toc do khong") - THEM @lru_cache. DO THAT (che do Modal remote):
    # encode 1 cau ton ~1.09s va KHONG he re hon khi lap lai CUNG 1 cau (khong cache -> moi lan
    # deu la 1 vong mang day du). TRAKE/Temporal goi LAI DUNG cung anchor text 2 LAN moi lan
    # search (pool goc coarse_k, roi pool mo rong widen_k o Level-2 backfill - xem dense_temporal
    # .py::_build_pools) -> lang phi dung 1.09s x so_anchor moi truy van (2 anchor = 2.2s, 4
    # anchor = 4.4s) cho ket qua Y HET NHAU. Encode la HAM THUAN TUY (cung text + cung model ->
    # cung vector), cache lai an toan tuyet doi. Cung nguyen tac da ap dung cho query_distill.py
    # ::distill_query (@lru_cache(maxsize=2048)) vi dung ly do.
    v = ENCODERS[model](query)
    v = v / (np.linalg.norm(v, axis=1, keepdims=True) + 1e-8)
    # Cache tra ve CUNG 1 object numpy cho moi caller -> khoa write de 1 caller lo sua nham
    # khong am tham dau doc cac lan goi sau (moi caller hien tai chi DOC: faiss index.search(),
    # sub_matrix @ qvec[0], qvec[0].tolist() - khong cho nao ghi).
...
def _combine_candidates(
    *,
    ...
    object/OCR/ASR toan cuc (giong filter.apply() nhung key la frame_idx)."""
...
def search_dense(
    ...
    *,
    ...
    man hinh (giong filter.by_text) - None = khong loc, "" cung coi nhu None.
    authors/date_from/date_to/keywords_any/video_ids: loc video (filter.by_metadata, dung
    ...
    ocr_algorithm: key trong filter.OCR_MATCH_ALGORITHMS ("flexible" mac dinh - nguoi
