"""Encode 1000 anh + 100 query qua SigLIP (Modal), luu embedding, tinh Recall@k + margin - so
truc tiep voi CLIP baseline (embeddings_clip.pkl) tren CUNG bo du lieu.

Chay: python offline/benchmark/eval_100way_siglip.py
"""
...
EMB_OUT_PATH = _Path(__file__).resolve().parent / "embeddings_siglip.pkl"
...
def main() -> None:
    ...
    Enc = modal.Cls.from_name("aic2026-siglip", "SigLIPEncoder")
    enc = Enc()
    ...
    print_report("SigLIP2 (google/siglip2-base-patch16-224)", r, t1 - t0 + t2 - t1)
